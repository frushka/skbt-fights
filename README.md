...
> **Актуальная архитектура (обновлено).** Передача данных в реальном времени переехала с
> Supabase Realtime на собственный WebSocket-сервер (`server/`). Причина: российские провайдеры
> глушат трансграничное соединение с `supabase.co` — поток затухает молча, без разрыва и без
> ошибки, и зал с мобильным интернетом остаётся без связи. Развёртывание и протокол описаны в
> `server/README.md`. Ниже — исходное задание, по которому проект создавался.
